...
inline cell amx_ftoc(float x)
{
	return *(cell*)&x;
}

inline float amx_ctof(cell x)
{
	return *(float *)&x;
}
...
template<size_t N>
const char *getAmxStringIP(AMX *amx, cell addr, char (&dest)[N], size_t *len = nullptr)
{
	char *source = getAmxString(getAmxAddr(amx, addr), dest, N - 1, len);
	if (source)
	{
		// strip port
		char *pos = Q_strchr(source, ':');
		if (pos) {
			*pos = '\0';
		}
	}
